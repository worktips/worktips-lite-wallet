...
const char DEFAULT_WALLET_FILE_NAME[] = "worktipswallet.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("worktipswallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("worktipswallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("worktipswallet.desktop");
  ...
  res = autorunSettings.contains("BytecoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("WorktipsWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.worktips.worktipswallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Worktips Lite\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("WorktipsWallet", appPath);
    } else {
      autorunSettings.remove("WorktipsWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\worktips", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:worktips");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\worktips\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\worktips\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("worktipswallet.cfg"));
